    ...
    Ownable
{
    ...
    modifier checkValue(uint256 _upc) {
        ...
        address payable consumerIDPayable = address(
            uint160(items[_upc].consumerID)
        );
        ...
        checkValue(_upc)
    {
        ...
        address payable originFarmerIDPayable = address(
            uint160(items[_upc].originFarmerID)
        );
        originFarmerIDPayable.transfer(msg.value);
        // emit the appropriate event
        emit Sold(_upc);
    }
